...
<body>
	<div class="container-fluid">
		...
		<div class="main-content">
			<div class="top-tips">
				<h5>
					您好，<%=cust.getFullname()%>。以下信息为您当前所报名的班级情况
				</h5>
				<p>您可以直接使用网络缴费，也可以点击现场根据提示时间到校缴费;</p>
			...
			<div class="tables">
				<table class="table table-striped table-hover">
					...
					<tfoot>
						<tr>
							<td>学费合计：</td>
							<td class="total" colspan="5">${paytotal }</td>
						</tr>
					</tfoot>
				</table>
					...
					<tbody>
						<c:forEach items="${notpays }" var="item">
							<tr id="course-${item.regclassid}">
								...
								<td>${item.classtime }</td>
								<td>${item.qty }</td>
								<td>${item.rcMoney }<input value="${item.fee }"
									type="hidden" id="fee-${item.regclassid}"></td>
								<td><c:choose>
										<c:when test="${item.ispay == 1 }">
												已缴费
												</c:when>
								...
								<td class="td-button"><a
									onclick="cancelCourse('${item.regclassid}')"
									href="javascript:;" class="btn btn-danger">退班</a></td>
							</tr>
						</c:forEach>
					</tbody>
					<tfoot>
		...
		<div class="btn-wrap xs-btn-group">
			<a href="javascript:;" class="btn btn-orange btn-lg"
				onclick="wechatPay()">微信支付</a> <a href="javascript:;"
				class="btn btn-default btn-lg"
				onclick="location.href='<%=basePath%>thirdStep'">继续报选班级</a>
		</div>
	</div>
