...
<style>
	...
	function getCourse(pageNum){
		...
		$.post("getCourse",{depid:depid,week:week,isfull:isfull,selectIds:selectIds,pageNum:pageNum,pageSize:pageSize,search:search},function(data){
			if(data!=null){
	...
	function addCourse(classid,classname,classtime,teacherid,qty,fee,dictionaryid,regclassid) {
		...
		//判断人数是否已满和时间冲突
			...
			if(data.code>0){
				...
				$("#totalpay").html(parseFloat($("#totalpay").html())+parseFloat(truefee))
			}else{
				alert(data.msg);
			}
	...
	/*function showPage(totalData){
		$('.paging').pagination({
			...
			callback: function (api) {
				...
				//调用ajax 请求数据回来 填进table里面


				var json = [
					{ name: "中医养生", time: "星期四上午", teacher: "张三", people: 12, tuition: '1230' },
					{ name: "中医养1生", time: "星期四上午", teacher: "张三", people: 12, tuition: '1230' },
					{ name: "中医养2生", time: "星期四上午", teacher: "张三", people: 12, tuition: '1230' },
					{ name: "中医养3生", time: "星期四上午", teacher: "张三", people: 12, tuition: '1230' },
					{ name: "中医养4生", time: "星期四上午", teacher: "张三", people: 12, tuition: '1230' },
				]
				...
				for (var i = 0; i < json.length; i++) {
					ele_array.push('<tr>')
					ele_array.push('<td>' + json[i].name + '</td>')
					ele_array.push('<td>' + json[i].time + '</td>')
					ele_array.push('<td>' + json[i].teacher + '</td>')
					ele_array.push('<td>' + json[i].people + '</td>')
					ele_array.push('<td class="money-color">' + json[i].tuition + '</td>')
					ele_array.push('<td><a href="javascript:;" class="btn btn-orange">选择</a></td>')
					ele_array.push('<td><a href="javascript:;" class="btn btn-primary">查看</a></td>')
					ele_array.push('</tr>')
				}
			...
			<div class="con-content">
				...
				<div class="main-content">
						...
						<div class="row">
							<div class="col-md-12">
								...
								<div class="right-content" style="margin-bottom: 10px;">
									<input placeholder="按班级名称或序号搜索" id="search" type="text" class="form-control" style="width: 50%;float: left;margin-right: 10px;">
					...
					<div class="btn-wrap">
						...
						<a onclick="checkCourse()"  href="javascript:;" class="btn btn-default btn-lg" >下一步</a>
						<a onclick="toHistory()" href="javascript:;" class="btn btn-default btn-lg ">报读原班</a>
					</div>
				</div>
			</div>
		</div>
