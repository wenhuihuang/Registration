...
<style>
	...
	function getCourse(pageNum){
		...
		$.post("getCourse",{week:week,isfull:isfull,selectIds:selectIds,pageNum:pageNum,pageSize:pageSize,search:search},function(data){
		...
		},"json").error(function(){alert("获取活动列表失败，请重试。")});
	}
	...
	function addCourse(classid,classname,classtime,teacherid,qty,fee,dictionaryid,regclassid) {
		...
		}).error(function(){alert("增加报名活动失败，请重试。")});
...
<script type="text/javascript">
	...
	function checkCourse(){
		if(confirm("是否确认报名选择的活动？")){
			...
			}).error(function(){alert("保存报名活动失败，请重试。")});			
		}
	}
</script>
</html>